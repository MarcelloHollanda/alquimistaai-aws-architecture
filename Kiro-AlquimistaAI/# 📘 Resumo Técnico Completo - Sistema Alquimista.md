...
\- **\*\*Trigger\*\***: EventBridge (schedule\_requested)  
\- **\*\*Timeout\*\***: 30s  
\- **\*\*Memory\*\***: 512 MB
...
1\. **\*\*001\_create\_schemas.sql\*\*** \- Criação dos schemas  
2\. **\*\*002\_create\_leads\_tables.sql\*\*** \- Tabelas Nigredo  
3\. **\*\*003\_create\_platform\_tables.sql\*\*** \- Tabelas Alquimista  
4\. **\*\*004\_create\_core\_tables.sql\*\*** \- Tabelas Fibonacci  
5\. **\*\*005\_create\_approval\_tables.sql\*\*** \- Sistema de aprovação  
6\. **\*\*006\_add\_lgpd\_consent.sql\*\*** \- Conformidade LGPD  
7\. **\*\*007\_create\_nigredo\_schema.sql\*\*** \- Schema completo Nigredo
...
\- score (0\-100)  
\- source, campaign\_id  
\- enrichment\_data (JSONB)  
\- created\_at, updated\_at  
\`\`\`

**\*\*campaigns\*\*** \- Campanhas de prospecção  
\`\`\`sql  
\- campaign\_id (UUID, PK)  
\- tenant\_id (UUID, FK)  
\- name, description  
\- target\_segment (JSONB)  
...
\`\`\`sql  
\- interaction\_id (UUID, PK)  
\- lead\_id (UUID, FK)  
\- type (message\_sent, message\_received, call, meeting)  
...
**\*\*agent\_activations\*\*** \- Agentes ativos por tenant  
\`\`\`sql  
\- activation\_id (UUID, PK)  
\- tenant\_id (UUID, FK)  
\- agent\_id (TEXT)  
\- status (active, inactive)  
\- config (JSONB)  
\- activated\_at, deactivated\_at  
\`\`\`

**\*\*audit\_logs\*\*** \- Logs de auditoria  
\`\`\`sql  
\- log\_id (UUID, PK)  
\- tenant\_id (UUID, FK)  
\- user\_id (UUID)  
...
\- approver\_id (UUID)  
\- decision\_reason (TEXT)  
\- created\_at, decided\_at  
\`\`\`
...
1\. **\*\*initial\_data.sql\*\*** \- Dados iniciais  
2\. **\*\*001\_production\_data.template.sql\*\*** \- Template para produção  
3\. **\*\*002\_default\_permissions.sql\*\*** \- Permissões padrão  
4\. **\*\*003\_internal\_account.sql\*\*** \- Conta interna
...
const result \= await db.query('SELECT \* FROM nigredo.leads WHERE tenant\_id \= $1', \[tenantId\]);  
\`\`\`
...
{  
  ...
  "detail-type": "schedule\_requested",  
  "detail": {  
    "tenantId": "123",  
    "leadId": "456",  
...
lgpd\_consent BOOLEAN DEFAULT FALSE  
lgpd\_consent\_date TIMESTAMP  
lgpd\_consent\_ip TEXT  
\`\`\`
...
\`\`\`bash  
POWERTOOLS\_SERVICE\_NAME\=nome-do-servico  
LOG\_LEVEL\=INFO|DEBUG  
EVENT\_BUS\_NAME\=fibonacci-bus-{env}  
DB\_SECRET\_ARN\=arn:aws:secretsmanager:...  
...
\`\`\`bash  
\# Disparo  
DLQ\_URL\=https://sqs.us-east-1.amazonaws.com/.../nigredo-dlq-dev

\# Atendimento  
SENTIMENT\_LAMBDA\_ARN\=arn:aws:lambda:...  
\`\`\`
...
NEXT\_PUBLIC\_REGION\=us-east-1  
\`\`\`
...
\`\`\`typescript  
const CIRCUIT\_BREAKER\_CONFIG \= {  
...
\# Seed database  
psql \-h \<host\> \-U dbadmin \-d fibonacci \-f database/seeds/initial\_data.sql  
\`\`\`
...
│   │   ├── 001\_create\_schemas.sql  
│   │   ├── 002\_create\_leads\_tables.sql  
│   │   ├── 003\_create\_platform\_tables.sql  
│   │   ├── 004\_create\_core\_tables.sql  
│   │   ├── 005\_create\_approval\_tables.sql  
│   │   ├── 006\_add\_lgpd\_consent.sql  
│   │   └── 007\_create\_nigredo\_schema.sql  
│   └── seeds/                    \# Seed data  
│       ├── initial\_data.sql  
│       ├── 001\_production\_data.template.sql  
│       ├── 002\_default\_permissions.sql  
│       └── 003\_internal\_account.sql  
│  
...
4\. Lambda → EventBridge (nigredo.recebimento.new\_lead)  
...
6\. Lambda → EventBridge (nigredo.disparo.message\_sent)  
\`\`\`
...
2\. Lambda → EventBridge (nigredo.atendimento.schedule\_requested)  
...
\- Todos os dados têm \`tenant\_id\`  
\- Queries sempre filtram por \`tenant\_id\`  
\- Cognito User Pool com custom attributes

**\*\*Segurança\*\***:  
\- JWT token contém \`tenant\_id\`  
\- Lambda valida \`tenant\_id\` em cada request  
